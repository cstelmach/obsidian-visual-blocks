...
1. **Enable the plugin.** Open Obsidian → Settings → Community plugins. The "Render Cache" plugin should appear in the installed-plugins list (Obsidian discovers `.obsidian/plugins/obsidian-render-cache/manifest.json`). Toggle it ON. Optional but recommended: in the Plugin Groups palette, REMOVE TikZJax from any active group — TikZJax also registers a `tikz` codeblock processor and last-registered wins (Obsidian-internal load-order question). With TikZJax disabled, no fence conflict.
...
3. **AC8.2 — Cached TikZ block displays inline.** Open `kn/math/concepts/mSB3-4_reals.md` in reading mode. The TikZ number-line diagram should appear EXACTLY ONCE (plugin emits `<img>`; legacy wikilink is hidden by plugin CSS). Compare to Phase 7 reading mode where you saw "codeblock + wikilink image" stacked — that should be replaced by a single image now.
...
8. **Cleanup after gate.** Remove the experimental ```d2\nx -> y\n``` block you added in step 4 (or run `render_cache.py` on the file to render it properly).
