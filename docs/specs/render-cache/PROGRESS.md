...
**Status:** Phase 6 user gate closed; Phase 7 (SVG postprocessing hardening) agent-side complete — awaiting user gate.
**Mode:** Manual (user-driven phase progression)
**Started:** 2026-04-27
**Last Updated:** 2026-04-27 (Phase 6 gate closed, Phase 7 agent-side done)
...
| Phase 7 — Apply SVG postprocessing hardening | DONE (agent) | 2026-04-27 15:10 | 2026-04-27 (this session) | 88a487fc9 (PROGRESS+residual) + a9cd7320c (auto-backup: code+tests+105 cache SVGs) | 43/43 Phase 7 ✓ + 151/151 full suite | ~30m | Three rules in `render_cache/postprocess.py` (`prefix_ids` / `substitute_current_color` / `enforce_viewbox`). Quote-agnostic regexes — PLAN's pseudocode pinned double-quote only and would have silently no-op'd on TikZ + SMILES (both single-quoted). Added CSS-style colour rule for SMILES (`style='...stroke:#000000...'`). Re-rendered 169 cache SVGs via `--all --force`; AC7.1/AC7.2/AC7.3 hard-verified across full cache (0 unprefixed dvisvgm or Graphviz IDs; 0 attribute-form OR CSS-style hardcoded black; 169/169 viewBox; 0/169 pt units). 3 pre-existing TikZ source bugs surfaced for the 3rd time (not Phase 7 regressions). |
...
### Phase 7 — Apply SVG postprocessing hardening — 2026-04-27 (this session) DONE (agent-side)
...
**Phase 7 gate (user-driven, per SPEC §11.4):**

This phase has TWO gate criteria; the `grep`-verifiable ones (AC7.1/AC7.2/AC7.3) are agent-confirmed. The visual ones require Obsidian:

1. **AC7.4 — Multi-block visual** — Open a note that has at least 2 cached SVGs from the same renderer family on one page. Confirm: each diagram's elements stay inside their own bounds (no corrupted edges, no missing labels, no element bleeding from one block into another). Easiest test: `kn/math/concepts/_RENDER_TEST_d2.md` (3 D2 blocks; visible together in reading mode).
2. **AC7.5 — Dark-mode follow** — In Obsidian, toggle to dark mode (Cmd+Shift+D or via Settings → Appearance). Open a TikZ note with text labels (e.g., `kn/math/concepts/mSB3-4_reals.md`). Confirm: the diagram's foreground (axis lines, labels) inverts to a light colour appropriate for the dark theme. The previous behaviour (Phase 1–6) was hardcoded black, which appears almost-invisible against a dark theme background.
...
When confirmed, reply: **"Implement Phase 8"** → Plugin scaffold (Node.js TypeScript plugin; NEW tooling environment; depends on Phase 7's clean cached SVGs).
